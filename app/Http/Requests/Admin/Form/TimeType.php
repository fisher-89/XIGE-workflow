...
trait TimeType
{
    ...
    protected function getTimeTypeRule($key,$field,$gridIndex)
    {
        $this->key = $key;
        ...
        $timeRule = [
            ...
            'fields.' . $key . '.min' => [
                'date_format:H:i:s',
                'before_or_equal:' . ($field['max'] ?: '23:59:59')
            ],
            'fields.' . $key . '.max' => [
                'date_format:H:i:s',
                'after_or_equal:' . ($field['min'] ?: '00:00:01')
            ],
            'fields.' . $key . '.scale' => [
                'required',
                'in:0'
        ...
        if(!is_null($this->gridIndex)){
            $gridTimeRule = [];
            foreach($timeRule as $k=>$v){
                $k = 'grids.'.$this->gridIndex.'.'.$k;
                $gridTimeRule[$k] = $v;
            }
            $timeRule = $gridTimeRule;
        }
        $this->timeMessage();
        return $timeRule;
    }
